<?php
	...
	public function addMenu($data) {
		...
		$this->db->query("INSERT INTO " . DB_PREFIX . "menu SET parent_id = '" . (int)$data['parent_id'] . "', chossen_id = '" . (int)$chossen . "', category_id_chossen = '" . $value . "', `top` = '" . (isset($data['top']) ? (int)$data['top'] : 0) . "', `column` = '" . (int)$data['column'] . "', sort_order = '" . (int)$data['sort_order'] . "', status = '" . (int)$data['status'] . "', date_modified = NOW(), date_added = NOW()");
	...
	public function editMenu($menu_id, $data) {
		...
		else if ((int)$data['chossen_id'] == 8) //day la category
		{
			if(	(int)$data['category_id_chossen_info'] )
			{
				$value= (int)$data['category_id_chossen_info'];
		...
		}else if ((int)$data['chossen_id'] == 9)
		{
			$chossen = $data['chossen_id'];
			$value= 'allnews';
		}else if ((int)$data['chossen_id'] == 10)
		{
			$chossen = $data['chossen_id'];
			$value= 'allinformation';
		}
	
		$this->db->query("UPDATE " . DB_PREFIX . "menu SET parent_id = '" . (int)$data['parent_id'] . "', `top` = '" . (isset($data['top']) ? (int)$data['top'] : 0) . "', chossen_id = '" . (int)$chossen . "', category_id_chossen = '" . $value . "', `column` = '" . (int)$data['column'] . "', sort_order = '" . (int)$data['sort_order'] . "', status = '" . (int)$data['status'] . "', date_modified = NOW() WHERE menu_id = '" . (int)$menu_id . "'");
